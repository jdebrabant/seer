...
public class WorkloadGenerator2D extends WorkloadGenerator
{
    ...
    public WorkloadGenerator2D(int minimum_x, int maximum_x, int minimum_y, int maximum_y, 
                               int x_partitions, int y_partitions, 
                               int num_sequences, String output_dir)
    {
		...
		p_space = new PredictionSpace2D(x_partitions, y_partitions); 
		((PredictionSpace2D)p_space).mapToDataSpace(minimum_x, maximum_x, minimum_y, maximum_y); 
